...
namespace AnService_Capstone.DataAccess.Dapper.Repositories
{
    public class MaterialRepository : IMaterialRepository
    {
        ...
        {
            var query = "select UsedMaterialID, used.MaterialID, used.RequestDetailID, used.Status, WorkerID, Quantity, QuantityNew, ServiceName, Note, Message, ServiceRequestDescription, CustomerName, mate.MaterialID, MaterialName, Unit, UserID, FullName, PhoneNumber, Address " +
                ...
                "join tblRequestDetails details on used.RequestDetailID = details.RequestDetailID) " +
            ...
            {
                connection.Open();
                var res = await connection.QueryAsync<MaterialViewModel, TblMaterial, UserViewModel, MaterialViewModel>(query, (requestService, material, user) =>
                {
                    requestService.Material = material;
                    requestService.Worker = user;
                    return requestService;
                }, param: new { @RequestDetailID = id }, splitOn: "MaterialID, UserID");
                connection.Close();
                if (res.Count() == 0)
                {
                    return null;
        ...
        {
            var query = "select UsedMaterialID, used.MaterialID, used.Status, Quantity, QuantityNew, ServiceName, Note, Message, mate.MaterialID, MaterialName, Unit, UserID, FullName " +
            ...
            using (var connection = _dapperContext.CreateConnection())
            {
                ...
                var res = await connection.QueryAsync<MaterialViewModel, TblMaterial, UserViewModel, MaterialViewModel>(query, (requestService, material, user) =>
                {
                    ...
                    requestService.Worker = user;
                    return requestService;
                }, param: new { @ServiceRequestID = id }, splitOn: "MaterialID, UserID");
                connection.Close();
                /*if (res.Count() == 0)
                {
                    return null;
        ...
        {
            var query = "select UsedMaterialID, used.MaterialID, used.RequestDetailID, used.Status, WorkerID, Quantity, Message, ServiceRequestDescription, CustomerName, mate.MaterialID, MaterialName, Unit, UserID, FullName, PhoneNumber, Address " +
                "from (((tblUsedMaterial used join tblMaterial mate on used.MaterialID = mate.MaterialID) join tblUsers us on used.WorkerID = us.UserID) " +
                "join tblRequestDetails details on used.RequestDetailID = details.RequestDetailID) " +
                "join tblServiceRequest rs on rs.ServiceRequestID = details.ServiceRequestID";
        ...
        public async Task<MaterialViewModel> GetRequestMaterialByID(int id)
        {
            ...
            using (var connection = _dapperContext.CreateConnection())
            {
                ...
                var res = await connection.QueryAsync<MaterialViewModel, TblMaterial, UserViewModel, MaterialViewModel>(query, (requestService, material, user) =>
                {
                    ...
                    requestService.Worker = user;
                    return requestService;
                }, param: new { @UsedMaterialID = id}, splitOn: "MaterialID, UserID");
                connection.Close();
                if (res.Count() == 0)
                {
                    return null;
